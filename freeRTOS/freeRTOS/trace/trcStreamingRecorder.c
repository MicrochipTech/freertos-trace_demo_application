...
void vTraceStoreKernelObjectName(void* object, const char* name)
{
	...
	prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME, name, (Pointer_UBaseType_t)object);
}
...
traceString xTraceRegisterString(const char* name)
{
	...
	/* Always save in symbol table, if the recording has not yet started */
	prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME, (const char*)name, (Pointer_UBaseType_t)name);
...
void vTraceStoreISRBegin(traceHandle handle)
{
	...
	{
		ISR_stack_index++;
		ISR_stack[ISR_stack_index] = (Pointer_UBaseType_t)handle;
#if (TRC_CFG_INCLUDE_ISR_TRACING == 1)
		prvTraceStoreEvent1(PSF_EVENT_ISR_BEGIN, (Pointer_UBaseType_t)handle);
#endif
...
void vTraceStoreISREnd(int isTaskSwitchRequired)
{
	...
	else
	{
		...
		{
#if (TRC_CFG_INCLUDE_ISR_TRACING == 1)
			prvTraceStoreEvent1(PSF_EVENT_TS_RESUME, (Pointer_UBaseType_t)TRACE_GET_CURRENT_TASK());
#endif
		}
	}
...
static void prvSetRecorderEnabled(uint32_t isEnabled)
{
	...
	if (RecorderEnabled)
	{
        ...
        prvTraceStoreEvent3(	PSF_EVENT_TRACE_START,
							...
							(Pointer_UBaseType_t)currentTask,
							SessionCounter++);
			...
			if (event != NULL)
			{
				...
				for (i = 0; i < nArgs; i++)
				{
					if ((userEvtChannel != NULL) && (i == 0))
					{
						...
						data32[i] = (Pointer_UBaseType_t)userEvtChannel;
					}
					else
					{
						/* Add data arguments... */
	...
	if (RecorderEnabled)
	{
...

		{
			...
			if (event != NULL)
			{
				...
				{
					/* First, add the User Event Channel if not NULL */
					data32[0] = (Pointer_UBaseType_t)userEvtChannel;
				}
...
void prvTraceSaveSymbol(const void *address, const char *name)
{
	...
	for (i = 0; i < firstFreeSymbolTableIndex; i += SYMBOL_TABLE_SLOT_SIZE)
	{
		...
		if (*ptrAddress == (Pointer_UBaseType_t)address)
		{
			foundSlot = i;
			break;
		}
	...
	{
		/* We access the symbol table via the union member pSymbolTableBufferUINT32 to avoid strict-aliasing issues */
		symbolTable.SymbolTableBuffer.pSymbolTableBufferUINT32[foundSlot / sizeof(uint32_t)] = (Pointer_UBaseType_t)address;
...
void prvTraceDeleteSymbol(void *address)
{
	...
	for (i = 0; i < firstFreeSymbolTableIndex; i += SYMBOL_TABLE_SLOT_SIZE)
	{
		...
		if (*ptr == (Pointer_UBaseType_t)address)
		{
...
void prvTraceSaveObjectData(const void *address, uint32_t data)
{
	...
	{
		/* We access the data table via the union member pObjectDataTableBufferUINT32 to avoid strict-aliasing issues */
		objectDataTable.ObjectDataTableBuffer.pObjectDataTableBufferUINT32[foundSlot / sizeof(uint32_t)] = (Pointer_UBaseType_t)address;
